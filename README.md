...
supabase/migrations/003_increment_and_check_usage_rpc.sql  -- 원자적 사용량 확인 RPC
```

> ⚠️ `001_initial_schema.sql`은 초기 설계 단계의 정규화 스키마입니다. 현재 운영 스키마는 `schema.sql` 기반이므로 001은 실행하지 마세요.
...
# 개발/테스트 계정 UUID (일일 한도 제외, 쉼표 구분)
# DEV_BYPASS_USERS=uuid1,uuid2
```
