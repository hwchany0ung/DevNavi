...
| 기능 | 설명 |
|------|------|
| 🗺️ **AI 로드맵 생성** | Claude Sonnet으로 월별·주별 세분화된 학습 계획 실시간 스트리밍 |
...
|------------|--------|------|------|
| `/health` | GET | ❌ | 헬스체크 |
| `/roadmap/teaser` | POST | ❌ | 미리보기 로드맵 스트리밍 (Claude Haiku) |
| `/roadmap/full` | POST | ✅ | 전체 로드맵 스트리밍 (Claude Sonnet) |
| `/roadmap/career-summary` | POST | ✅ | 커리어 분석 JSON |
| `/roadmap/persist` | POST | ✅ | 스트리밍 완료 후 DB 저장 |
| `/roadmap/reroute` | POST | ✅ | GPS 재탐색 스트리밍 |
| `/roadmap/my` | GET | ✅ | 내 최신 로드맵 ID 조회 |
| `/roadmap/activity/me` | GET | ✅ | 365일 활동 데이터 (잔디) |
| `/roadmap/{id}` | GET | ✅ | 로드맵 상세 조회 (본인 소유) |
| `/roadmap/{id}/completions` | POST/GET | ✅ | 태스크 완료 토글/조회 |
...
> ⚠️ `VITE_SUPABASE_ANON_KEY`는 Supabase 설계상 프론트엔드에 공개되는 키입니다.
