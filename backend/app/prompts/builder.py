...
if TYPE_CHECKING:
    from app.models.roadmap import OnboardingSkillItem, ExtraProfile
...
def _format_skills_with_levels(skills) -> str:
    """스킬 리스트를 SKIP/REVIEW/FOCUS 3구간으로 분류한 프롬프트 텍스트 반환.

    skills: list[OnboardingSkillItem] 또는 list[str] (하위호환).
    """
    if not skills:
        return "보유 스킬: 없음"

    # str 리스트인 경우 (하위호환) — 기존 동작 유지
    if skills and isinstance(skills[0], str):
        return "보유 스킬: " + ", ".join(skills) if skills else "보유 스킬: 없음"

    # OnboardingSkillItem 리스트 — 3구간 분류
    skip_items = []   # advanced / intermediate → 스킵/최소화
    review_items = [] # basic → 빠른 복습
    focus_items = []  # beginner → 집중 학습

    for s in skills:
        name = s.name if hasattr(s, "name") else str(s)
        level = (s.level.value if hasattr(s, "level") and hasattr(s.level, "value")
                 else str(getattr(s, "level", "basic")))
        kr = _LEVEL_KR.get(level, level)
        tag = f"{name}({kr})"

        if level in ("advanced", "intermediate"):
            skip_items.append(tag)
        elif level == "basic":
    ...
    parts = []
    if skip_items:
        parts.append(f"  - 스킵/최소화 구간 (이미 보유, 심화·프로젝트 활용): {', '.join(skip_items)}")
    if review_items:
        parts.append(f"  - 빠른 복습 구간 (기초 지식 있음, 1~2주 복습 후 실전): {', '.join(review_items)}")
    if focus_items:
        parts.append(f"  - 집중 학습 구간 (처음부터 체계적 학습 필요): {', '.join(focus_items)}")

    if not parts:
        return "보유 스킬: 없음"

    return "보유 스킬 (숙련도별 3구간 분류):\n" + "\n".join(parts)


def _format_extra_profile(ep) -> str:
    """ExtraProfile 정보를 프롬프트 텍스트로 변환. None이면 빈 문자열."""
    if ep is None:
        return ""
    lines = ["\n[추가 프로필 — 로드맵 개인화 참고]"]
    lines.append(f"실배포 경험: {'있음' if ep.has_deployment else '없음'}")

    ct_map = {"none": "없음", "basic": "기초 (Easy 수준)", "intermediate": "중급 (Medium 수준)", "advanced": "고급 (Hard 수준)"}
    lines.append(f"코딩테스트 준비 수준: {ct_map.get(ep.coding_test_level, ep.coding_test_level)}")

    tp_map = {0: "없음", 1: "1회", 2: "2회", 3: "3회 이상"}
    lines.append(f"팀 프로젝트 경험: {tp_map.get(ep.team_project_count, str(ep.team_project_count))}")

    return "\n".join(lines)


def _skills_to_str(skills) -> str:
    """skills를 쉼표 구분 문자열로 변환 (기존 동작 호환).

    list[OnboardingSkillItem] → "React, Python, ..."
    list[str] → "React, Python, ..."
    """
    if not skills:
        return "없음"
    names = []
...
필수 규칙:
- 보유 스킬 숙련도별 3구간 분류가 제공된 경우:
  - 스킵/최소화 구간 (능숙/중급): 해당 스킬 학습은 건너뛰거나 최소화, 심화 프로젝트에 바로 활용
  - 빠른 복습 구간 (기초): 1~2주 빠른 복습 후 실전 적용
  - 집중 학습 구간 (입문/미보유): 처음부터 체계적 학습, 로드맵 핵심 배치
- 숙련도 정보가 없는 경우: 보유 스킬은 건너뛰고 부족한 부분에 집중
- 추가 프로필(실배포·코딩테스트·팀프로젝트 경험)이 제공된 경우 로드맵에 적극 반영
...
[규칙]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- skills_to_learn: 보유 스킬(특히 능숙/중급) 제외, 4~6개 (priority 1부터 채울 것)
- 숙련도별 3구간 분류가 제공된 경우: 입문/미보유 스킬에 높은 priority, 기초 스킬은 심화 방향 제안
...
async def build_teaser_prompt(role: str, period: str, level: str) -> tuple[str, str]:
...
async def build_full_prompt(
    ...
    skills_section = _format_skills_with_levels(skills)
    certs_str = ", ".join(certifications) if certifications else "없음"
    extra_section = _format_extra_profile(extra_profile)
...
태스크 content 길이: {content_len_hint} (엄수){extra_section}"""

    return FULL_SYSTEM, user


async def build_full_prompt_partial(
    role: str,
    level: str,
    skills,
    certifications: list[str],
...
async def build_career_summary_prompt(
    role: str,
    period: str,
    level: str,
    skills,
...
{skills_section}
보유 자격증: {certs_str}
목표 회사 유형: {company_kr}{extra_section}"""

    return CAREER_SUMMARY_SYSTEM, user


async def build_reroute_prompt(
    original_role: str,
    original_period: str,
    company_type: str,
    ...
    company_kr = COMPANY_MAP.get(company_type, "무관")
    tasks_per_week = TASKS_PER_WEEK.get(daily_study_hours, 3)
    reference = await get_reference(original_role)
...
[원래 목표]
직군: {role_kr}
목표 회사: {company_kr}{user_requests_section}"""
