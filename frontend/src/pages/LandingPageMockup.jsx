...
function SectionLabel({ children, dark }) {
  return (
    <p className={`text-xs font-bold tracking-widest uppercase mb-3 ${dark ? 'text-indigo-400' : 'text-indigo-500'}`}>
      {children}
    </p>
  )
}
...
function HeroPreview({ dark }) {
  ...
  /* 라이트/다크 카드 스타일 */
  const cardBg    = dark ? 'bg-[#0d0f1a]'       : 'bg-white'
  const cardBdr   = dark ? 'border-white/10'     : 'border-slate-200'
  const topBarBg  = dark ? 'bg-[#13151f]'        : 'bg-slate-50'
  const topBarBdr = dark ? 'border-white/10'     : 'border-slate-200'
  const monoMuted = dark ? 'text-white/30'       : 'text-slate-400'
  const progBg    = dark ? 'bg-white/10'         : 'bg-slate-100'
  const progBdr   = dark ? 'border-white/10'     : 'border-slate-100'
  const itemBg    = dark ? 'bg-white/[0.03]'     : 'bg-slate-50'
  const itemBdr   = dark ? 'border-white/[0.05]' : 'border-slate-100'
  const taskText  = dark ? 'text-white/70'       : 'text-slate-700'
  const footerTxt = dark ? 'text-white/20'       : 'text-slate-300'
  const hintText  = dark ? 'text-white/25'       : 'text-slate-400'
  ...
  return (
    <div className="flex flex-col items-center gap-3 select-none">
      ...
      <p className={`text-xs ${hintText}`}>↑ 실제 생성 과정 미리보기</p>
    </div>
  )
}
      ...
      {dark
        ? /* 해 아이콘 */
          <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
            <path d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4.22 1.78a1 1 0 011.42 1.42l-.71.7a1 1 0 11-1.41-1.41l.7-.71zM18 9a1 1 0 110 2h-1a1 1 0 110-2h1zM4.22 16.22a1 1 0 001.42-1.42l-.71-.7a1 1 0 10-1.41 1.41l.7.71zM10 16a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zm-5.66-2.34a1 1 0 10-1.41-1.41l-.71.7a1 1 0 001.41 1.42l.71-.71zM3 10a1 1 0 110-2H2a1 1 0 100 2h1zm12.78-5.78a1 1 0 00-1.41 1.41l.7.71a1 1 0 001.42-1.42l-.71-.7zM10 6a4 4 0 100 8 4 4 0 000-8z" />
          </svg>
        : /* 달 아이콘 */
          <svg viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 z-50 bg-amber-500/90 backdrop-blur text-black text-xs font-bold
        ...
        <button onClick={() => navigate('/')} className="underline opacity-70 hover:opacity-100">
          현재 랜딩 보기
        </button>
      </div>
      ...
      <nav className={`px-6 py-5 flex items-center justify-between max-w-6xl mx-auto
        ${t('', 'border-b border-slate-200/60 bg-white/80 backdrop-blur sticky top-8 z-40')}`}>
        ...
        <div className="flex items-center gap-5">
          ...
          <button
            onClick={() => navigate('/')}
            className={`text-sm transition-colors hidden sm:block ${t('text-white/40 hover:text-white/70', 'text-slate-500 hover:text-slate-800')}`}>
            로그인
          </button>
      ...
      <section className="px-6 py-16 max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-14 lg:gap-20">
        ...
        <div id="preview" className="flex-shrink-0 flex flex-col items-center gap-3">
          ...
          <p className={`text-xs ${t('text-white/25', 'text-slate-400')}`}>↑ 클릭해서 탭 전환 가능한 실제 미리보기</p>
        </div>
      </section>
      ...
      <section id="problem" className={`px-6 py-20 max-w-6xl mx-auto border-t ${t('border-white/[0.06]', 'border-slate-200')}`}>
        <div className="text-center mb-14">
          <SectionLabel dark={dark}>이런 적 있으셨나요?</SectionLabel>
          ...
          {[
            {
              ...
              dark: 'bg-indigo-500/[0.07] border-indigo-500/15',
            ...
            },
            {
              ...
              dark: 'bg-violet-500/[0.07] border-violet-500/15',
            ...
            },
            {
              ...
              dark: 'bg-cyan-500/[0.07] border-cyan-500/15',
              light: 'bg-cyan-50 border-cyan-100',
            },
          ].map(({ emoji, title, desc, dark: darkCls, light: lightCls }) => (
            <div key={title} className={`rounded-2xl p-6 border ${dark ? darkCls : lightCls}`}>
      ...
      <section id="how" className={`px-6 py-20 max-w-6xl mx-auto border-t ${t('border-white/[0.06]', 'border-slate-200')}`}>
        <div className="text-center mb-14">
          <SectionLabel dark={dark}>DevNavi가 하는 일</SectionLabel>
      ...
      <section id="proof" className={`px-6 py-20 max-w-6xl mx-auto border-t ${t('border-white/[0.06]', 'border-slate-200')}`}>
        <div className="text-center mb-14">
          <SectionLabel dark={dark}>실제 사용 후기</SectionLabel>
          <h2 className="text-3xl sm:text-4xl font-black">
            "막막함이{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-cyan-500">
              사라졌어요
      ...
      <section className={`px-6 py-24 max-w-3xl mx-auto text-center border-t ${t('border-white/[0.06]', 'border-slate-200')}`}>
        ...
        <p className={`mt-5 text-xs ${t('text-white/25', 'text-slate-400')}`}>
          신용카드 불필요 · 언제든 삭제 가능
        </p>
      </section>
